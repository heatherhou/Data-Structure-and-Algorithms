...
public class Benchmark {
	...
	public static int uniqueKeys(String text, int order) {
		EfficientMarkov test = new EfficientMarkov(order);
...
//		}
	...
	public static void varykfixed(String source,int randomLength) throws Exception {
		...
		System.out.printf("order\t#chars\tource\tmean\tsigma\n");
	...
	public static int testRandomLength(String source, int randomLength, int order) {
		MarkovInterface<WordGram> model = getMarkov(order);
		model.setTraining(source);
		
		int result = 0;
		for (int i = 0; i < 10; i++)
		{
			result += model.getRandomText(randomLength).length();
...
////		
		...
		for(int order : orders) {
			for (int i = 2000; i <= 16000; i *= 2) {
				...
				System.out.println(uniqueKeys(source,order));
...
//		for (int i = 0; i<10;i++)
//		{
//			textLen = testRandomLength(source,5000, order);
//			System.out.printf("%d\t%1.3f",source.length(),data[0]);
//			System.out.println(textLen);
//			
//		}
//		
		
//		String[] fileNames = {"alice.txt", "hawthorne.txt", "kjv10.txt", "poe.txt", "melville.txt", "romeo.txt","hamlet.txt","shakespeare.txt"};
...
//				
//				int uniqueKeys = uniqueKeys(text,elem);
//				data = benchmark(randomLength, elem, text);
